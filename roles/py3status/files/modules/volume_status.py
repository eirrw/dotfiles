...
class Wpctl(Audio):
    def setup(self, parent):
        self.is_default = (self.device == None)
        if self.is_default:
            self.device = "@DEFAULT_AUDIO_SOURCE@" if self.is_input else "@DEFAULT_AUDIO_SINK@"

        if not self.is_default and not self.device.isnumeric():
            self.device = get_device_id()

    def get_device_id(self):
        output = self.command_output(["pw-dump", "-N"])
        pw_array = json.loads(output)

        node = next((i for i in pw_array if i["info"]["props"]["node.name"] == self.device), None)
        if node == None:
            raise RuntimeError("Failed to find device {}", self.device)

        return node["id"]

    def get_volume(self):
        output = self.command_output(['wpctl', 'get-volume', self.device])
        vol = output.split()
        if len(vol) == 2:
            muted, perc = False, int(float(vol[1]) * 100)
        elif len(vol) == 3:
            muted, perc = True, int(float(vol[1]) * 100)
        else:
            muted, perc = None, None
    ...
    def volume_up(self, delta):
        ...
        if int(perc) + delta >= self.max_volume:
            ...
            change = f"{delta}%+"
        self.run_cmd(["wpctl", "set-volume", self.device, change])

    def volume_down(self, delta):
        self.run_cmd(["wpctl", "set-volume", self.device, f"{delta}%-"])

    def toggle_mute(self):
        self.run_cmd(["wpctl", "set-mute", self.device, "toggle"])
...
class Py3status:
    ...
    def post_config_hook(self):
        if not self.command:
            commands = ["wpctl", "pamixer", "pactl", "amixer"]
        ...
        elif self.command not in ["wpctl", "amixer", "pamixer", "pactl"]:
