...
class Py3status:
    ...
    def _get_bluez_data(self):
        ...
        batteries = temporary.pop("batteries", [])
        for device in temporary.pop("devices", []):
            for index, _ in enumerate(batteries):
                if device["path"] == batteries[index]["path"]:
                    device.update(batteries.pop(index))
                    break
